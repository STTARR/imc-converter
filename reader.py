...
def parse_channel_headers(headers: Sequence[str]) -> List[str]:
...
def xyzc_to_arr(arr: np.ndarray) -> np.ndarray:
...
def read_txt(path: Union[Path, str]) -> xr.DataArray:
    ...
    channel_names = parse_channel_headers(txt.columns[3:])
    img = xyzc_to_arr(txt.values).astype(np.float32)
    return xr.DataArray(img, name=Path(path).stem, dims=("y", "x", "c"),
        coords={"x": range(img.shape[1]), "y": range(img.shape[0]), 
                "c": channel_names})


def read_mcd(path: Union[Path, str], encoding: str="utf-16-le") -> List[xr.DataArray]:
    """Read a Fluidigm IMC .mcd file and return as a list of xarray DataArray
    ...
    with open(path, mode="rb") as fh:
        ...
        # Parse xml, force Acquisition(Channel) to be list even if one item so we can iterate
        ...
        for acq in acquisitions:
            ...
            imgs.append(
                xr.DataArray(img, name=f"{Path(path).stem}_{id_}",
                    dims=("y", "x", "c"), 
                    coords={"x": range(img.shape[1]), "y": range(img.shape[0]),
                            "c": channel_names},
                    attrs=acq,
